...
export default function Home() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const resp = await fetch("/ask-question", {
      method: "POST",
  ...
  async function streamResponse(resp: Response) {
    ...
    reader?.read().then(async function processText({
      done,
      value,
    }): Promise<void> {
      if (done) return;
    ...
    <div className="flex justify-center p-12">
      <main className="max-w-4xl w-full">
        <form onSubmit={handleSubmit} className="mb-5">
          ...
          <button
            type="submit"
            className="bg-blue-700 hover:bg-blue-800 focus:ring-blue-900 text-xl rounded-md py-5 w-full"
          >
            Submit
          </button>
        </form>
